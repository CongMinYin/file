...
class ServerManager(base.BootingManagerWithFind):
    ...
    #def ping(self, server=None, network=None, interface=None, ip=None):
    def ping(self, server=None, net_id=None, ips=None):
        '''
        ping a server until
        '''
        if ips :
            ip=ips[list(ips.keys())[0]][0]['addr']
        else :
            return 0
        '''
        command = 'ip netns'
        ...
        command = 'ssh new ip netns exec {1} ping {0} -c 1'.format(ip, ip_net)
        start = time.time()
        while True:
            r = os.popen(command)
            r_str = r.readlines()
            if str(r_str).find("1 received") != -1 :
                return 0
            elif (time.time() - start) > 300:
                return -1
        # time out arrive here
        '''
        with open('/home/cmyin/rallytest.txt','w') as f:
            f.write('name:' + str(server.name) + '\n')
            f.write('ip:' + str(ip) + '\n')
        ...
        f = open('/home/cmyin/rallytest.txt','w')
        f.write('name:' + str(server.name) + '\n')
        f.write('ips:' + str(ips) + '\n')
        f.write('ip:' + str(ip) + '\n')
        f.write('ip_net:' + ip_net + '\n')

        command = 'ip netns exec {1} ping {0} -c 1'.format(ip, ip_net)
        r = os.popen(command)
        r_str = r.readlines()
        f.write('result:' + str(r_str) + '\n')
        if str(r_str).find("1 received") != -1 :
            f.write('result:ping arrive' + '\n')
        else:
            f.write('result:ping loss' + '\n')
        ...
        command = 'ip netns exec qdhcp-25bee494-8157-4d5f-b9c0-505be24881fc ping {1} -c 1'.format(ip, '192.168.2.2')
        r = os.popen(command)
        r_str = r.readlines()
        f.write('result:' + str(r_str) + '\n')
        f.write('result:' + str(str(r_str).find('1 received')) + '\n')
        if str(r_str).find("1 received") != -1 :
            f.write('result:ping arrive' + '\n')
        else:
            f.write('result:ping loss' + '\n')

        command='echo ' + str(server.name) + ' > /home/cmyin/rallytest.txt'
        r = os.popen(command)
        command='echo ' + str(server.accessIPv4) + ' >> /home/cmyin/rallytest.txt'
        r = os.popen(command)
        f.close()
        '''
        return 0
